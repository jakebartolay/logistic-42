                    ...
                    <div class="row gy-4 gy-sm-1">
                      <div class="col-sm-6 col-lg-3">
                        <div
                          ...
                          <div>
                            <h4 class="mb-2" id="pending-count">{{ $pendingCountOutbound }}</h4>
                      ...
                      <div class="col-sm-6 col-lg-3">
                        <div
                          ...
                          <div>
                            <h4 class="mb-2" id="completed-count">{{ $completedCountOutbound }}</h4>
                      ...
                      <div class="col-sm-6 col-lg-3">
                        <div
                          ...
                          <div>
                            <h4 class="mb-2" id="refunded-count">{{ $packedCountOutbound }}</h4>
                            <p class="mb-0 fw-medium">Packed</p>
                          </div>
                      ...
                      <div class="col-sm-6 col-lg-3">
                        <div class="d-flex justify-content-between align-items-start">
                          <div>
                            <h4 class="mb-2" id="failed-count">{{ $forInvoiceCountOutbound }}</h4>
                            <p class="mb-0 fw-medium">For Invoice Creation</p>
                          </div>
              ...
              <div class="card-datatable table-responsive">
                  <table class="datatables-order table border-top">
                      ...
                      <tbody>
                          ...
                          <tr>
                              <td></td>
                              <td>{{ $order->order_id_outbound }}</td>
                              <td>{{ $order->client_name_outbound }}</td>
                              <td>{{ $order->contact_number_outbound }}</td>
                              <td>{{ $order->payment_method_outbound }}</td>
                              <td>{{ $order->shipping_address_outbound}}</td>
                              <td>{{ $order->shipping_date }}</td>
                              <td>{{ $order->status_outbound }}</td>
                              <td>
                              <button class="action_button" id="createInvoiceButton" data-order-id-outbound="{{ $order->order_id_outbound }}">
                ...
                <br><br><center><div id="pagination">
                    <!-- Previous button -->
                    @if ($outboundOrders->previousPageUrl())
                        <a href="{{ $outboundOrders->previousPageUrl() }}" id="prev-page">&lt; &nbsp;&nbsp;</a>
                    @endif
                    
                    <!-- Current page number -->
                    <span id="current-page">{{ $outboundOrders->currentPage() }}</span>
                    
                    <!-- Next button -->
                    @if ($outboundOrders->nextPageUrl())
                        <a href="{{ $outboundOrders->nextPageUrl() }}" id="next-page">&nbsp;&nbsp; &gt;</a>
                    @endif
                </div></center>
        <!--/ Content -->
            <!-- Footer -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Add event listeners to all buttons with class 'action_button'
        const buttons = document.querySelectorAll('.action_button');
        buttons.forEach(button => {
            button.addEventListener('click', function() {
                // Retrieve the order ID from the button's data attribute
                const orderId = this.getAttribute('data-order-id-outbound');
                
                // Send a GET request to fetch the outbound data for the specific order ID
                axios.get('{{ route("get.outbound.data", ["orderId" => ":orderId"]) }}'.replace(':orderId', orderId))
                    .then(function(response) {
                        // Redirect to the invoice page with the outbound data as query parameters
                        localStorage.setItem('outboundData', JSON.stringify(response.data));
                        window.location.href = '{{ route("invoice") }}?data=' + encodeURIComponent(JSON.stringify(response.data));
